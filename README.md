...
- **「非同期通信(async await)」** を用いてデータベースとのやりとりを行い、ユーザーストレスの軽減をしました。
- **「react-dropzone」と「react-cropper」** を用いてみんながいつも使っている SNS ように写真を好きな様に切り取ってプロフィールに反映できるようにしました。
- **「Cloud Function」** を用いてサーバーレスアーキテクチャにしました。その結果、バックエンドの工数を削減を実現し、前もってデプロイしてあった api を実行する関数を必要な時だけ叩けるようになりました。
- **テストユーザー** に使ってもらってフィードバックを反映している
- **firestore.rules**で Firestore の読み書きを制限している
- Hooks を使っており、**カスタムフック**も活用している
...
- ユーザー登録・編集・削除
- SNS ログイン・ユーザー登録(Facebook, Email)
- ゲストログイン
